...
from gigachat_api import get_access_token, sent_promt_and_get_response
...
if "access_token" not in st.session_state:
    try:
        ...
        st.toast(f"Не удалость получить токен: {e}")
...
if user_promt := st.chat_input():
    ...
    with st.spinner("В процессе..."):
        response, is_image = sent_promt_and_get_response(user_promt, st.session_state.access_token)
        if is_image:
